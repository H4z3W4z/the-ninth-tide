...
`tests/maps_render.gd` requires a real renderer. The GitHub workflow uses Xvfb and software OpenGL to exercise native mouse/touch event dispatch and capture five review screens. Screenshots are published as the `map-review` workflow artifact. These desktop checks do not establish actual iPad/Safari behavior.
